...
func renderPlanHuman(plan *planner.Plan, dag *planner.DAG) string {
	if plan.TotalIssues == 0 {
		return render.EmptyState("No issues to plan.", "Create issues first with: sonar issue create", false)
	}
...
// sortDeps sorts formatted IDs (e.g. "SNR-7", "SNR-12") by their numeric value.
